...
const VoiceChat = () => {
  ...
  const startRecording = async () => {
    try {
      ...
      // Try wav or webm depending on browser support
      ...
      setIsRecording(true);
      setStatus('🎤 Listening... speak in Odia now!');
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-slate-900 text-white">
      {/* Header */}
      <div className="bg-black/40 backdrop-blur border-b border-slate-700 p-4">
        <h1 className="text-2xl font-bold text-amber-400">🎤 Utkal Voice Chat</h1>
        <p className="text-sm text-slate-400">Speak Odia. Get answers in authentic Odia script and voice.</p>
      </div>
        ...
        {messages.length === 0 ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-lg text-slate-300 font-medium">ମାଇକ୍ରୋଫୋନ୍ ବଟନ୍ ଦବାଇ କଥା ହୁଅନ୍ତୁ</p>
              <p className="text-sm text-slate-500 mt-1">Press the button below to start speaking in Odia</p>
            </div>
          </div>
        ) : (
          messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.type === 'user' ? 'justify-end' : 'justify-start'}`}>
              ...
              >
                <div className="text-xs text-slate-300 mb-1 font-semibold">
                  {msg.type === 'user' ? '👤 You / ଆପଣ' : '🤖 Utkal AI / ଉତ୍କଳ ଏଆଇ'}
                </div>
      ...
      {/* Controls */}
      <div className="bg-black/40 backdrop-blur border-t border-slate-700 p-6 flex justify-center items-center gap-4">
        <button
